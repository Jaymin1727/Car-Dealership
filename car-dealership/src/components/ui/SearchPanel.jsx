...
import styles from '../../styles/ui/SearchPanel.module.css';
...
export default function SearchPanel() {
  ...
  useEffect(() => {
    const handler = (e) => {
      if (e.key === 'Escape') setSearchPanel(false);
    };
  ...
  const toggleFilter = (chip) => {
    ...
    const next = activeFilters.includes('All')
      ...
      : activeFilters.includes(chip)
        ? activeFilters.filter((f) => f !== chip).length === 0
          ? ['All']
          : activeFilters.filter((f) => f !== chip)
        : [...activeFilters, chip];
      ...
      >
        <div className={styles.inputRow}>
          ...
          >
            ...
            <ChevronDown
              size={14}
              className={`${styles.chevron} ${showFilters ? styles.chevronUp : ''
                }`}
            />
          </button>
        </div>
        ...
        <div className={styles.chips}>
          {FILTER_CHIPS.map((chip) => (
            <motion.button
              key={chip}
              className={`${styles.chip} ${activeFilters.includes(chip) ||
                  (chip === 'All' && activeFilters.includes('All'))
                  ? styles.chipActive
                  : ''
                }`}
