...
function CarModel({ started }) {
  ...
  useEffect(() => {
    ...
    scene.traverse((child) => {
      ...
      if (name.includes('body') || name.includes('hood') || name.includes('chassis') || name.includes('car')) {
        child.material = new THREE.MeshStandardMaterial({ color: new THREE.Color(0x0a0a0a), metalness: 0.95, roughness: 0.05, envMapIntensity: 1.8 });
      }
      if (name.includes('glass') || name.includes('window') || name.includes('windshield')) {
        child.material = new THREE.MeshStandardMaterial({ color: new THREE.Color(0x334455), metalness: 0.1, roughness: 0, transparent: true, opacity: 0.35 });
      }
      if (name.includes('wheel') || name.includes('tire') || name.includes('tyre')) {
        wheelsRef.current.push(child);
        child.material = new THREE.MeshStandardMaterial({ color: new THREE.Color(0x111111), metalness: 0.2, roughness: 0.8 });
      }
      if (name.includes('rim') || name.includes('spoke')) {
        child.material = new THREE.MeshStandardMaterial({ color: new THREE.Color(0xcccccc), metalness: 0.9, roughness: 0.1 });
      }
  ...
  useFrame(() => {
    ...
    wheelsRef.current.forEach((w) => { w.rotation.x = t * 1.5; });
  });
...
function SceneLighting() {
  return (
    <>
      ...
      <directionalLight position={[8, 10, 5]} intensity={1.2} castShadow shadow-mapSize={[2048, 2048]} />
      <directionalLight position={[-6, 4, -4]} intensity={0.4} color="#4488ff" />
      <pointLight position={[0, 6, 0]} intensity={0.6} />
      <spotLight position={[0, 8, 4]} angle={0.3} penumbra={0.8} intensity={1.5} castShadow />
...
export default function DashboardHero() {
  ...
  const stats = [
    { icon: Car,        label: 'Total Models', value: vehicles.length,                              color: '#3b82f6' },
    { icon: Package,    label: 'In Stock',      value: totalStock,                                   color: '#10b981' },
    { icon: TrendingUp, label: 'Revenue',       value: `$${(totalRevenue || 0).toLocaleString()}`,   color: '#f59e0b' },
      ...
      <div className={styles.inner}>
        ...
        >
          ...
          >
            ...
            <h1 className={styles.greetName}>{firstName} <span>👋</span></h1>
            <p className={styles.greetDesc}>Manage your fleet, track performance, and keep inventory sharp.</p>
          </motion.div>
          ...
          <div className={styles.statsGrid}>
            {stats.map(({ icon: Icon, label, value, color }, i) => (
              ...
              >
                <span className={styles.statIcon} style={{ color, background: `${color}22` }}>
                  <Icon size={16} />
                </span>
          ...
          >
            ...
            <Suspense fallback={<FallbackCar />}>
              <OrbitControls
                ...
                autoRotateSpeed={1.2}
              />
              <Float speed={1.5} rotationIntensity={0.08} floatIntensity={0.18} floatingRange={[-0.03, 0.03]}>
                <CarModel started={started} />
              </Float>
            </Suspense>
            <ContactShadows position={[0, -0.95, 0]} opacity={0.5} scale={12} blur={2.5} far={4} color="#000000" />
